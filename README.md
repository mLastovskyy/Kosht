...
- **A code is trusted only once it proves itself.** Loyalty cards and adverts share the same square; a payload counts as a receipt when it carries fiscal fields or leads to a page an amount can be read from. A ЕРИП payment code is turned away by name, and a link that yields nothing no longer leaves an empty record behind.
- **The Belarusian code is an identifier, not a link.** Most tills here print the 24 characters that also appear as `УИ` on the paper, and nothing else. Kosht reads them: for shops on iKassa tills the whole receipt — shop, lines, total — comes back from that identifier alone. Евроопт's `r.eplus.by` code is answered with the PDF of its e-receipt, and a printed PDF reads far cleaner than a photograph of paper.
